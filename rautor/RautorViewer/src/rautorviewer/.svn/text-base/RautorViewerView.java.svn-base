 ...
 */
    ...
    private void initComponents() {
        ...
        jMenuItem7.setAction(actionMap.get("ChooseFTPSession")); // NOI18N
    ...
    public void DispImage(int Index){        
        ...
        if ( Files == null ){
               statusMessageLabel.setText("No PNG Images here ?");
            return;
        }
        if ( Index > VideoLast) {
...
// **************************************
    ...
    public void PickSession() {
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            busyIconTimer.stop(); statusAnimationLabel.setIcon(idleIcon);
            return;
        }
        VideoSlider.setEnabled(false);
        StartReplayButton.setEnabled(false);
 ...
 */
  public void doFtpWork(FileObject Sessiondir){                
        int i=0;
        FileObject LocalFiles[]=VFSUtils.getFiles(Sessiondir);
        String FriendlyPath=VFSUtils.getFriendlyName(Sessiondir.toString());
        if ( LocalFiles ==null)  {
            statusMessageLabel.setText("No PNG files found under "+FriendlyPath);
            busyIconTimer.stop();
            statusAnimationLabel.setIcon(idleIcon);
            return;
        }
        for ( FileObject filename : LocalFiles){
...
public String ReadScreenContents(int i) {
      ...
      if ( SessionIsRemote){
          try {              
              ...
              statusMessageLabel.setText(friendlyname); Thread.yield();
       ...
       @Override
        public void run() {
            while(true) {
               ...
               Thread.yield();
               try {  Thread.sleep(1333); } catch ( Exception e) {
                   statusMessageLabel.setText(e.getLocalizedMessage());
               }
              }
