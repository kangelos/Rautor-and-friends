...
&UnsetProxy();
Win32::MsgBox("Proxy settings are now removed!", 0|MB_ICONINFORMATION, "Rautor");
1;
    ...
    if(-d "$ENV{APPDATA}\\Mozilla\\Firefox\\Profiles"){
        ...
        foreach my $pdir (@pdirs){
		   ...
		   #	Write out the rest of the stuff
	    	...
	    	# now re-enable  the tools menu!
	   if ( -f "$mozdir\\$pdir\\chrome\\userChrome.css.rautor" ){ 
		File::Copy(	"$mozdir\\$pdir\\chrome\\userChrome.css.rautor",
				"$mozdir\\$pdir\\chrome\\userChrome.css");		
	   } else {
	   	if ( -f "$mozdir\\$pdir\\chrome\\userChrome.css" ){ 
		   unlink("$mozdir\\$pdir\\chrome\\userChrome.css");
		}
         }
